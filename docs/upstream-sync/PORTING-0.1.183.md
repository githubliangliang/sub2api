...
### 4.1 `49752060`（v0.1.182）+ `b20f29d1`（v0.1.183）渠道监控 v2 里 composite 分组的错误全部丢失 —— **已合**（2026-08-31，`98db82b57`）

2 文件 +34-13 与 2 文件 +3-1。**两条必须合成一处改动**：`b20f29d1` 修的正是
`49752060` 写出的 `NULLIF(TRIM(a.platform))`（`NULLIF` 少了第二个参数，是硬 SQL 错误）。
状态：**已合**（2026-08-31）。实际是约 10 行 SQL 语义搬运——这是本文档里最后一个、也是唯一一个
从头到尾没被量过规模的项；量完就发现「需 SQLite 重写」听起来重、实际只是照本节给出的表达式改两处。
配套新增 `channel_monitor_v2_classify_sqlite_test.go`（**真跑 SQL**，见 0.1.184 §25）。
