...
- [PORTING-0.1.184.md](./PORTING-0.1.184.md) —— 上游 0.1.184 大混合版（2026-08-31）。**§3 的 14 项 P0 与 §4 的 11 项 P1 已全部合入**（实施记录见其 §9 / §11，验证矩阵见 §10 / §12；3.11 为部分合）。上游 3 条新迁移全部不合，故本仓库迁移号仍是 224。剩下的是 §4 那组「取决于是否用到该功能」的按需项 + §5 的六簇，其中 §5.1 Codex routed catalog「未发布」的阻塞条件已解除、需单独立项。
