...
[PORTING-0.1.184.md](./PORTING-0.1.184.md) 那轮添了三条通用教训，移植前值得先看：

- **`apply --check` 干净 ≠ 能编译/能跑**，那一轮抓到三条（0.1.184 §2 与 §11.1）。判定方法不是看三态，而是对新增代码里的每个函数调用逐个 grep 基座是否存在。**前端同样成立**：`ed12ea716` 六文件全干净却直接 `escapeTomlBasicString is not defined`，而 `pnpm run typecheck` 抓不到 —— 移植前端必须真跑 vitest。
- **逐条 cherry-pick 会落在上游的中间态。** 同一个函数的签名、同一条用例的入参，可能在区间里更靠后的提交又改过一次（0.1.184 §9.3 / §11.3）。移植前先 `git show <tag>:<file>` 看终态，再决定按哪一版落；也要留意「看着该跟着改、上游其实刻意没改」的地方。
- **「缺基座」要顺着符号往上游历史回溯一层，先量基座本身有多大**（0.1.184 §16）。零命中只说明「现在做不了」，不说明「做起来贵」——§5.7 当初按零命中归入不做，实际基座是两条各百行以内、都能干净落地的漏项。按这条回头量了剩余全部「缺基座」判断，**四处全部低估**（0.1.184 §17）：`decodeOpenAIJSONUseNumber` 是 24 行 stdlib-only 独立文件、`IsOpenAIOAuthLike` 是 3 行谓词、§5.2 的基座约 50 行。0.1.183 §5.1 Responses Lite 簇由此重新可做。
- **基座要随用它的簇一起移植**，不要为了「先补齐」单独落一个没人调用的函数（0.1.184 §17.2）——那是死代码。免费的基座是「解锁条件已满足」的信号，不是「可以先合」的许可。
- **backlog 里挂最久的项，往往是最早被贴上标签、之后再没人量过的那个**（0.1.184 §25）。「需 SQLite 重写」「缺基座」这类标签一旦写下就被后续每一轮沿用。定期回头量一遍最老的几项，比继续往前找新项划算——0.1.183 §4.1 从第一天挂到最后，量完只是约 10 行 SQL。
- **SQL 常量的注释里不要写 PG 语法字面量**，哪怕当反例——`sqlite_dialect_audit_test` 连注释一起扫（0.1.184 §25.2）。
- **用例整取之后仍可能有个别子用例越界**（0.1.184 §23.1）。第 13 条的 grep 保证「能编译」，保证不了「断言的都是本条改的行为」。子用例失败时先问「这条断言的是本条改的东西吗」，再决定补基座还是删用例。
- **同一条规则在本仓库的另一条路径上可能已经写对了**（0.1.184 §22.2）。移植前 grep「这个语义在别处怎么处理」，既能确认缺陷真实存在，也能拿到现成的判定写法——`c83dced4b` 那条 WARN 一边打印 `close_status=1000` 一边把错误算到账号账上，日志里就写着答案。
- **「测试文件 apply 干净」不等于「测试能编译」**（0.1.184 §21.2）。三态是按文件报的，测试文件通常无冲突，很容易被记成「测试没问题、只差产品代码」——`f4e3eb1c5` 的两个用例就是 apply 干净但引用了未移植的符号。判定方法同第一条：对用例引用的符号逐个 grep。
- **试落时如果只有测试文件干净、产品代码全冲突，立刻撤回测试文件**（0.1.184 §20.2）。留着会得到「编译通过但用例必然失败」的状态，比没开始更糟，下一轮还容易误以为已合。撤回时把每个产品 hunk 的行数量下来，下一轮照表手工对齐。
- **移植基座之后 grep 一遍谁还在用被它取代的旧写法**（0.1.184 §20.1）——`decodeOpenAIJSONUseNumber` 落地后发现 `prepareOpenAIWSHTTPBridgeBody` 还在用 `json.Unmarshal` 静默丢大整数精度，这个缺陷不在任何清单里。
