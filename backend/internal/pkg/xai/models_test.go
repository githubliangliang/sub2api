...
func TestExplicitGrok45DoesNotFollowRuntimeDefault(t *testing.T) {
	...
	require.Equal(t, "grok-4.5", ResolveGrokTextResponsesModelID("grok-4.5-latest", "grok-4.3"))
...
func TestDefaultModelsOmitsGrok3MiniIDs(t *testing.T) {
	t.Parallel()
	ids := DefaultModelIDs()
	require.NotContains(t, ids, "grok-3-mini")
	require.NotContains(t, ids, "grok-3-mini-fast")
	require.Equal(t, "grok-3-mini", ResolveGrokTextResponsesModelID("grok-3-mini"))
	require.Equal(t, "grok-3-mini-fast", ResolveGrokTextResponsesModelID("grok-3-mini-fast"))
}
