...
func BenchmarkReadLargeBody(b *testing.B) {
	for _, size := range []int{512 << 10, 69 << 20} {
		body := bytes.Repeat([]byte{'x'}, size)
		b.Run(fmt.Sprintf("bytes_%d/legacy_buffer", size), func(b *testing.B) {
			b.ReportAllocs()
			b.SetBytes(int64(size))
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				buf := bytes.NewBuffer(make([]byte, 0, requestBodyReadMaxInitCap))
				_, err := io.Copy(buf, bodyReaderOnly{bytes.NewReader(body)})
				if err != nil || !bytes.Equal(buf.Bytes(), body) {
					b.Fatalf("legacy body changed: %v", err)
				}
			}
		})
		b.Run(fmt.Sprintf("bytes_%d/chunked", size), func(b *testing.B) {
			b.ReportAllocs()
			b.SetBytes(int64(size))
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
