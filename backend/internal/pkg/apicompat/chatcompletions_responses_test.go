...
func TestResponsesEventToChatChunks_DoneCompletesOnlyMissingArgumentSuffix(t *testing.T) {
	for _, typ := range []string{"function_call", "custom_tool_call"} {
		t.Run(typ, func(t *testing.T) {
			state := NewResponsesEventToChatState()
			state.SentRole = true
			ResponsesEventToChatChunks(&ResponsesStreamEvent{Type: "response.output_item.added", Item: &ResponsesOutput{Type: typ, CallID: "call-1", Name: "exec"}}, state)
			...
			ResponsesEventToChatChunks(&ResponsesStreamEvent{Type: deltaType, Delta: `{"id":`}, state)
			event := &ResponsesStreamEvent{Type: doneType, Arguments: `{"id":1}`, Input: `{"id":1}`}
