...
func TestResolveCompositeModelOwnershipKeepsProviderAccountsIsolated(t *testing.T) {
	...
	require.Equal(t, CompositeModelOwnership{TargetPlatform: PlatformGrok, Matched: true}, deepSeekOwnership)
...
func TestResolveCompositeModelOwnershipAllowsSamePlatformAndRejectsCrossPlatformAliases(t *testing.T) {
	...
	repo := &compositeOwnershipAccountRepo{
		accounts: []Account{
			...
			{ID: 3, Platform: PlatformGrok, Credentials: map[string]any{"model_mapping": map[string]any{"ambiguous": "deepseek-v4-pro"}}},
		},
	}
	svc := &GatewayService{accountRepo: repo}
