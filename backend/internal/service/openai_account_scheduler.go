...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if !req.DisableStickyEscape && account != nil && len(s.filterGrokFreeQuotaAccounts(ctx, []Account{*account})) == 0 {
		_ = s.service.deleteStickySessionAccountID(ctx, req.GroupID, sessionHash)
		return nil, false, nil
	}
	...
	if !req.DisableStickyEscape && account != nil && isGrokTeamModelRateLimited(account, upstreamModel, now) {
		_ = s.service.deleteStickySessionAccountID(ctx, req.GroupID, sessionHash)
		return nil, false, nil
	}
	if !req.DisableStickyEscape && account != nil && isGrokModelQuotaBlocked(account.ID, upstreamModel, now) {
		_ = s.service.deleteStickySessionAccountID(ctx, req.GroupID, sessionHash)
		return nil, false, nil
	}
	...
	if s.service.concurrencyService != nil {
		...
		if !req.DisableStickyEscape && s.hasAlternateOpenAICapacity(ctx, req, accountID) {
			_ = s.service.deleteStickySessionAccountID(ctx, req.GroupID, sessionHash)
			return nil, false, nil
		}
		return attachSelectionProfitGate(ctx, &AccountSelectionResult{
