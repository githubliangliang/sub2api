...
func upstreamModelRegistryBaseURL(account *Account) string {
	...
	switch {
	// 上游这里是 `account.IsOpenAI() || account.IsCNProvider()` + GetOpenAIFormatBaseURL()。
	// 本仓库没有国产供应商平台（domain/constants.go 只有 anthropic/openai/gemini/
	// antigravity/grok），也没有协议感知的 GetOpenAIFormatBaseURL —— 按本仓库形态落。
