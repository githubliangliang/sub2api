...
func TestResolveAntigravityForwardBaseURL_PaidUsesOfficialDailyHost(t *testing.T) {
	t.Setenv(antigravityForwardBaseURLEnv, "")
	paid := resolveAntigravityForwardBaseURL(&Account{Credentials: map[string]any{"plan_type": "pro"}})
	require.Equal(t, "https://daily-cloudcode-pa.googleapis.com", paid)
	require.NotContains(t, paid, ".sandbox.")
	free := resolveAntigravityForwardBaseURL(&Account{Credentials: map[string]any{"plan_type": "free"}})
	require.Equal(t, "https://cloudcode-pa.googleapis.com", free)
}
