...
func TestAccountSupportsOpenAIEndpointCapability(t *testing.T) {
	...
	t.Run("空 openai_capabilities（map[string]bool）与未配置一致", func(t *testing.T) {
		account := &Account{
			Platform: PlatformOpenAI,
			Type:     AccountTypeOAuth,
			Credentials: map[string]any{
		...
		caps, configured := account.openAIEndpointCapabilitySet()
	...
	t.Run("非空但全 false 的 map 仍按显式禁用处理，不默认放行", func(t *testing.T) {
		...
		caps, configured := account.openAIEndpointCapabilitySet()
		require.True(t, configured, "non-empty all-false must stay configured")
		require.False(t, caps["chat_completions"])
		require.False(t, account.SupportsOpenAIEndpointCapability(OpenAIEndpointCapabilityChatCompletions))
	})
