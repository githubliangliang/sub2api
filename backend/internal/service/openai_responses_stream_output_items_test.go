...
func TestNormalizeResponsesStreamingTerminalOutputKeepsUnknownAndDuplicateTypes(t *testing.T) {
	doneItems := newResponsesStreamOutputItems()
	doneItems.Observe([]byte(`{"type":"response.output_item.done","output_index":0,"item":{"id":"fc_1","type":"function_call","name":"a"}}`))
	doneItems.Observe([]byte(`{"type":"response.output_item.done","output_index":1,"item":{"id":"fc_2","type":"function_call","name":"b"}}`))
	doneItems.Observe([]byte(`{"type":"response.output_item.done","output_index":2,"item":{"id":"x_1","type":"vendor_widget","payload":{"k":1}}}`))
	...
	require.True(t, changed)
	require.Len(t, gjson.GetBytes(normalized, "response.output").Array(), 3)
	require.Equal(t, "function_call", gjson.GetBytes(normalized, "response.output.0.type").String())
	require.Equal(t, "fc_1", gjson.GetBytes(normalized, "response.output.0.id").String())
	require.Equal(t, "function_call", gjson.GetBytes(normalized, "response.output.1.type").String())
	require.Equal(t, "fc_2", gjson.GetBytes(normalized, "response.output.1.id").String())
	require.Equal(t, "vendor_widget", gjson.GetBytes(normalized, "response.output.2.type").String())
	require.Equal(t, float64(1), gjson.GetBytes(normalized, "response.output.2.payload.k").Float())
}
