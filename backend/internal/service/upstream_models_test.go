...
func TestBuildUpstreamModelsRequestSupportsOpenAIOAuth(t *testing.T) {
	...
	canonicalVersion := CodexCanonicalClientVersion()
	require.Equal(t, resolveCodexOutboundIdentity("").version, canonicalVersion)
	require.Equal(t, canonicalVersion, req.URL.Query().Get("client_version"))
	require.Equal(t, canonicalVersion, req.Header.Get("Version"))
