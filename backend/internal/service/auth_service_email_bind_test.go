...
func TestAuthServiceBindEmailIdentity_AllowsOwnAliasRewrite(t *testing.T) {
	cache := &emailBindCacheStub{
		data: &service.VerificationCodeData{
			Code:      "123456",
			CreatedAt: time.Now().UTC(),
	...
	require.NoError(t, err)
	currentUser := createEmailBindTestUser(t, client, "a.b@gmail.com", "own-alias", hashedPassword)

	updatedUser, err := svc.BindEmailIdentity(ctx, currentUser.ID, "ab@gmail.com", "123456", "current-password")
	require.NoError(t, err)
	require.NotNil(t, updatedUser)
	require.Equal(t, "ab@gmail.com", updatedUser.Email)

	storedUser, err := client.User.Get(ctx, currentUser.ID)
	require.NoError(t, err)
	require.Equal(t, "ab@gmail.com", storedUser.Email)
}
...
func TestAuthServiceSendEmailIdentityBindCode_LookupFailureIsUnavailable(t *testing.T) {
	cache := &emailBindCacheStub{
		data: &service.VerificationCodeData{
			Code:      "123456",
			CreatedAt: time.Now().UTC(),
	...
	userRepo.aliasErr = errors.New("alias lookup failed")
	cfg := &config.Config{
		JWT: config.JWTConfig{Secret: "test-bind-email-secret", ExpireHour: 1},
	}
	emailService := service.NewEmailService(nil, cache)
	svc := service.NewAuthService(nil, userRepo, nil, nil, cfg, nil, emailService, nil, nil, nil, nil, nil, nil)

	err := svc.SendEmailIdentityBindCode(context.Background(), user.ID, "other@gmail.com")
