...
func TestClaudeCodeValidator_MaxTokensOneProbeIsNotLimitedToHaiku(t *testing.T) {
	...
	for _, model := range []string{"claude-sonnet-4-5", "claude-opus-4-1", "claude-haiku-4-5"} {
		t.Run(model, func(t *testing.T) {
			...
			for _, mt := range []any{float64(1), 1, int64(1)} {
...
func TestClaudeCodeValidator_NonProbeValuesKeepStrictValidation(t *testing.T) {
	validator := NewClaudeCodeValidator()
	req := httptest.NewRequest(http.MethodPost, "/v1/messages", nil)
	req.Header.Set("User-Agent", "claude-cli/2.1.260 (external, cli)")
	for _, value := range []any{nil, 0, 2, -1, 1.5, "1", true} {
		require.False(t, validator.Validate(req, map[string]any{"model": "claude-sonnet-4-5", "max_tokens": value}), "max_tokens=%v (%T)", value, value)
	}
}
