...
func TestNormalizeCacheCreationBreakdown_BillingSafetyInvariant(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "aggregate 1000 with details 1000+1000 sums to 1000",
			tokens: UsageTokens{CacheCreationTokens: 1000, CacheCreation5mTokens: 1000, CacheCreation1hTokens: 1000},
			want5m: 500,
			want1h: 500,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.tokens.CacheCreationTokens > 0 && tt.tokens.CacheCreation5mTokens+tt.tokens.CacheCreation1hTokens > tt.tokens.CacheCreationTokens {
				require.Equal(t, tt.tokens.CacheCreationTokens, got5m+got1h)
			}
		})
	}
}
