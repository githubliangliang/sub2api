...
func TestOpenAIReasoningEffortPolicyFourPathsAgree(t *testing.T) {
	body := []byte(`{"reasoning":{"effort":"high"}}`)
	hooksClamp := &OpenAIWSIngressHooks{MaxReasoningEffort: "low"}
	hooksDeny := &OpenAIWSIngressHooks{MaxReasoningEffort: "low", MaxReasoningEffortOverLimit: ReasoningEffortOverLimitDeny}

	clamped, changed, err := ApplyOpenAIReasoningEffortPolicy(body, "low", nil, "")
	require.NoError(t, err)
	require.True(t, changed)
	require.Equal(t, "low", gjson.GetBytes(clamped, "reasoning.effort").String())

	wsClamped, err := applyOpenAIWSReasoningEffortPolicy(body, hooksClamp)
	require.NoError(t, err)
	require.Equal(t, clamped, wsClamped)

	ctx := WithOpenAIReasoningEffortPolicy(context.Background(), "low", nil, "")
	fromCtx, changed, err := ApplyOpenAIReasoningEffortPolicyFromContext(ctx, body)
	require.NoError(t, err)
	require.True(t, changed)
	require.Equal(t, clamped, fromCtx)

	_, _, err = ApplyOpenAIReasoningEffortPolicy(body, "low", nil, ReasoningEffortOverLimitDeny)
	require.Error(t, err)
	_, err = applyOpenAIWSReasoningEffortPolicy(body, hooksDeny)
	require.Error(t, err)
	denyCtx := WithOpenAIReasoningEffortPolicy(context.Background(), "low", nil, ReasoningEffortOverLimitDeny)
	_, _, err = ApplyOpenAIReasoningEffortPolicyFromContext(denyCtx, body)
	require.Error(t, err)
}
