...
func TestForwardAsChatCompletions_APIKeyAutoDerivesStableIsolatedPromptCacheKey(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{responses: []*http.Response{response(), response(), response(), response()}}
	...
	otherSessionBody := []byte(`{"model":"gpt-5.4","messages":[{"role":"system","content":"be concise"},{"role":"user","content":"a different session"}],"stream":false}`)
	...
	otherSessionKey := gjson.GetBytes(upstream.bodies[3], "prompt_cache_key").String()
	require.NotEmpty(t, firstKey)
	require.Equal(t, firstKey, appendedKey)
	require.NotEqual(t, firstKey, otherTenantKey)
	require.NotEqual(t, firstKey, otherSessionKey)
	...
	require.NotEqual(t, upstream.requests[0].Header.Get("session_id"), upstream.requests[3].Header.Get("session_id"))
}
