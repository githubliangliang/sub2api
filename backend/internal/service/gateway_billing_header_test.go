	...
	for _, endpoint := range []string{"messages", "count_tokens"} {
		for _, tc := range []struct {
			...
			t.Run(endpoint+"/"+tc.name, func(t *testing.T) {
				...
				// "hello world" supplies bytes 4='o', 7='o', and missing 20='0'.
				// Derive the expected suffix independently of the production body parser.
				sum := sha256.Sum256([]byte("59cf53e54c78oo0" + version))
