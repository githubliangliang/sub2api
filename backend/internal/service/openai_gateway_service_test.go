...
func TestOpenAIGatewayService_SessionHashCodexReconnectStaysStable(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
...
func TestOpenAIGatewayService_SessionHashHyphenWinsOverUnderscore(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodPost, "/openai/v1/responses", nil)
	...
	require.Equal(t, "hyphen-session", svc.ExtractSessionID(c, body))
	hyphenHash := svc.GenerateSessionHash(c, body)
	underscoreOnly := httptest.NewRequest(http.MethodPost, "/openai/v1/responses", nil)
	underscoreOnly.Header.Set("session_id", "underscore-session")
	cUnderscore, _ := gin.CreateTestContext(httptest.NewRecorder())
	cUnderscore.Request = underscoreOnly
	underscoreHash := svc.GenerateSessionHash(cUnderscore, body)
	hyphenOnly := httptest.NewRequest(http.MethodPost, "/openai/v1/responses", nil)
	hyphenOnly.Header.Set("session-id", "hyphen-session")
	cHyphen, _ := gin.CreateTestContext(httptest.NewRecorder())
	cHyphen.Request = hyphenOnly
	hyphenOnlyHash := svc.GenerateSessionHash(cHyphen, body)

	require.Equal(t, hyphenOnlyHash, hyphenHash, "both headers present must produce exactly the hyphen-value hash")
	require.NotEqual(t, underscoreHash, hyphenHash, "must not hash the underscore value when hyphen is present")
}
