...
func TestBuildCodexModelsManifestForGroupUsesProviderImageCapabilities(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			accounts: []Account{{
				ID: 16, Platform: PlatformGrok, Type: AccountTypeAPIKey,
			}},
			modalities: []any{"text"},
		},
		{
...
func TestBuildCodexModelsManifestForGroupLoadsAccountsOnce(t *testing.T) {
	...
	require.Contains(t, repo.platforms, PlatformComposite)
	require.NotContains(t, repo.platforms, "kimi")
	require.NotContains(t, repo.platforms, "zhipu")
	require.NotContains(t, repo.platforms, "deepseek")
}
...
func TestBuildCodexModelsManifestForGroupUsesFallbackWhenTextOnlyPlatformHasNoSnapshot(t *testing.T) {
	...
	body, err := svc.BuildCodexModelsManifestForGroup(
		context.Background(),
		&Group{ID: 733, Platform: PlatformGrok},
		"",
