...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_PassthroughBridgeBoundaries(t *testing.T) {
	tests := []struct {
		...
		// 上游这里还有一条 name:"malformed data" 的子用例（截断 JSON、wantRelayReject:true），
		// **本仓库已删**：它断言的是「透传 relay 拒收非法 JSON 首帧」，那是本仓库尚未移植的
		// relay 行为，不属 7c616db07 的改动范围。本条改动对截断 JSON 的判定
		// （json.Valid 为假 → 不桥接、留在透传）由
		// TestOpenAIWSPassthroughFirstMessageBridgeDecision 直接覆盖。
		{
			name:      "duplicate type",
