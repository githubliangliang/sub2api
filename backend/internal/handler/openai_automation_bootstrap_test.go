...
func TestCodexAutomationBootstrapAdmission(t *testing.T) {
	output := codexAutomationBootstrap("wiki-maintenance", "never", automationBootstrapPrompt)
	...
	t.Run("existing call_id is passed through unchanged", func(t *testing.T) {
		body := codexAutomationBootstrapBody(t, output, `,"call_id":"call-1"`)
	...
	t.Run("ordinary tool-call missing call_id is still rejected", func(t *testing.T) {
		body := []byte(`{"model":"gpt-5","input":[{"type":"function_call_output","namespace":"codex_app","name":"other","output":` + mustJSON(t, output) + `}]}`)
		got, accepted, rec := admitCodexCallOutputRequest(t, body)
		require.Equal(t, body, got)
