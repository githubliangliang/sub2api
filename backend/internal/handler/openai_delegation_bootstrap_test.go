...
// admitCodexCallOutputRequest runs the Responses admission sequence used by
// OpenAIGatewayHandler.Responses: bootstrap normalizers first, then the
// existing function_call_output call_id gate. Tests must not skip that gate.
func admitCodexCallOutputRequest(t *testing.T, body []byte) (normalized []byte, accepted bool, rec *httptest.ResponseRecorder) {
	...
	accepted = (&OpenAIGatewayHandler{}).validateFunctionCallOutputRequest(c, normalized, zap.NewNop())
	return normalized, accepted, rec
}

func TestCodexDelegationBootstrapAdmission(t *testing.T) {
	t.Run("missing call_id is accepted and not given a synthetic call_id", func(t *testing.T) {
		body := []byte(`{"model":"gpt-5","input":[{"type":"function_call_output","namespace":"codex_app","name":"create_thread","output":"` + delegationEnvelope + `"}]}`)
		got, accepted, rec := admitCodexCallOutputRequest(t, body)
	...
	t.Run("existing call_id is passed through unchanged", func(t *testing.T) {
		body := []byte(`{"model":"gpt-5","input":[{"type":"function_call_output","call_id":"call-1","namespace":"codex_app","name":"create_thread","output":"` + delegationEnvelope + `"}]}`)
		got, _, _ := admitCodexCallOutputRequest(t, body)
		require.Equal(t, body, got)
		require.Equal(t, "call-1", gjson.GetBytes(got, "input.0.call_id").String())
	})

	t.Run("ordinary tool-call missing call_id is still rejected", func(t *testing.T) {
		body := []byte(`{"model":"gpt-5","input":[{"type":"function_call_output","name":"shell","output":"done"}]}`)
		got, accepted, rec := admitCodexCallOutputRequest(t, body)
		require.Equal(t, body, got)
		require.False(t, accepted)
		require.Equal(t, http.StatusBadRequest, rec.Code)
		require.Contains(t, rec.Body.String(), "function_call_output requires call_id")
	})

	t.Run("client-supplied bootstrap flag does not relax ordinary tool-call admission", func(t *testing.T) {
		body := []byte(`{"model":"gpt-5","bootstrap":true,"input":[{"type":"function_call_output","name":"shell","output":"done"}]}`)
		got, accepted, rec := admitCodexCallOutputRequest(t, body)
		require.Equal(t, body, got)
		require.False(t, accepted)
		require.Equal(t, http.StatusBadRequest, rec.Code)
