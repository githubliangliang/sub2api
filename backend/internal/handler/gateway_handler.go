...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	for {
		...
		for {
			...
			account = latest
			selection.Account = latest
			// Keep the refreshed account for the deferred release.
			sessionSlotAccounts[account.ID] = account
			// 等待路径保持既有 eager 绑定（无门时 helper 直接绑定）；调度器已
			// 抢槽的直达路径无门时由选号内部绑定，这里只在门下补准入后绑定。
			if selection.ProfitGateActive() || !selection.Acquired {
...
// defaultCodexModelIDsForPlatform 给 Codex catalog 兜底模型列表。
//
// 上游在这里为 service.PlatformDeepseek 返回 {"deepseek-v4-pro","deepseek-v4-flash"}
// （见上游 195b21970）。本仓库没有国产供应商平台（domain/constants.go 只有 anthropic /
// openai / gemini / antigravity / grok），那一支落地即死代码，故只保留通用兜底。
// 将来若真加了国产平台，把那一支补回来即可——函数签名与调用点已就位。
