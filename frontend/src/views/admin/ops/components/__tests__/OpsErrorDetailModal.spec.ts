...
describe('OpsErrorDetailModal', () => {
  ...
  it('emits back when returning to the filtered list', async () => {
    mocks.getRequestErrorDetail.mockResolvedValue({
      id: 2,
      created_at: '2026-08-19T00:00:00Z',
      phase: 'request',
    ...
    const wrapper = shallowMount(OpsErrorDetailModal, {
      props: { show: true, errorId: 2, errorType: 'request', backToList: true },
      global: {
        stubs: {
    ...
    await wrapper.get('[data-testid="error-detail-back-to-list"]').trigger('click')
