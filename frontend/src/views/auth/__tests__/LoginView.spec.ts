...
describe('LoginView registration entry', () => {
  ...
  it('keeps registration hidden while public settings are pending', async () => {
    let finish!: (settings: typeof publicSettings) => void
    getPublicSettingsMock.mockReturnValueOnce(new Promise(resolve => { finish = resolve }))
    const wrapper = mountLogin()
    expect(wrapper.text()).not.toContain('auth.signUp')
    finish(publicSettings)
    await flushPromises()
