...
// escapeTomlBasicString 转义 TOML basic string 里的反斜杠与双引号。
//
// 移植说明：上游把这个 helper 放在 22e1b8144（Codex routed model catalog 簇，本仓库未合，
// 见 docs/upstream-sync/PORTING-0.1.184.md §5.1）里，而 ed12ea716（Codex API key 模式内联
// 鉴权）调用了它。ed12ea716 的 patch 在本仓库 apply --check 干净，合上去却直接
// `escapeTomlBasicString is not defined` —— 又一例「三态干净不等于能跑」，这次在前端。
// 这个函数本身与那个功能簇无关（TOML 转义是独立正确的行为），故就地补上 2 行，
// 不必为它去拖整簇。
